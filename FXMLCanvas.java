    ...
    public class FXMLCanvas implements Initializable {
        ...
        List<Noeud> nodeDisplay = new ArrayList<>(); // liste des noeuds S
        List<Bord> edgeDisplay = new ArrayList<>(); // liste de aretes A
        ...
        public void ResetHandle(ActionEvent event) {
            ...
            selectedNode = null;
            nodeDisplay = new ArrayList<Noeud>();
            edgeDisplay = new ArrayList<Bord>();
            addNode = true;
            addEdge = false;
            addEdgeButton.setDisable(true);
        ...
        public void PlayHandle(ActionEvent event) {
            ...
            if(nbNodes == 0) return;
            if(!started){
                Graphe graphe = new Graphe(nodeDisplay, edgeDisplay);
                t = new Thread() {
                    private volatile boolean stp = false;
                    @Override
                    public void run() {
                        GrapheComplet.TrouverCliqueMaximale(graphe);
                    }
            ...
            if (addNode) {
                ...
                if (ev.getSource().equals(canvasGroup)) {
                    if (ev.getEventType() == MouseEvent.MOUSE_RELEASED && ev.getButton() == MouseButton.PRIMARY) {
                        ...
                        Noeud circle = new Noeud(ev.getX(), ev.getY(), 1.2, String.valueOf(nbNodes));
    ...
    /* -------------------------------------------------------------------- */
        ...
        public class Noeud extends Circle{
            ...
            public Noeud(double x, double y, double rad, String name) {
    ...
    /* ------------------------------------------------------------------- */
        ...
        public class Bord extends Line {
            Noeud node1, node2;

            public Bord(Noeud node1, Noeud node2) {
                super(
    ...
    /* ---------------------------------------------------------------------- */
        ...
        boolean edgeExists(Noeud u, Noeud v) {
            for (Bord e : edgeDisplay) {
